<template>
  ...
  <XContainer :draggable="true" @remove="() => $emit('remove')">
    <template #header
      ><i class="fas fa-bolt"></i> {{ $ts._pages.blocks.button }}</template
    >
    ...
    <section class="xfhsjczc">
      ...
      <MkSwitch v-model="value.primary"
        ><span>{{ $ts._pages.blocks._button.colored }}</span></MkSwitch
      >
      <MkSelect v-model="value.action">
        <template #label>{{ $ts._pages.blocks._button.action }}</template>
        <option value="dialog">
          {{ $ts._pages.blocks._button._action.dialog }}
        </option>
        <option value="resetRandom">
          {{ $ts._pages.blocks._button._action.resetRandom }}
        </option>
        <option value="pushEvent">
          {{ $ts._pages.blocks._button._action.pushEvent }}
        </option>
        <option value="callAiScript">
          {{ $ts._pages.blocks._button._action.callAiScript }}
        </option>
      </MkSelect>
      ...
      <template v-else-if="value.action === 'pushEvent'">
        ...
        <MkSelect v-model="value.var">
          ...
          <option :value="null">
            {{ $t("_pages.blocks._button._action._pushEvent.no-variable") }}
          </option>
          <option v-for="v in hpml.getVarsByType()" :value="v.name">
            {{ v.name }}
          </option>
          <optgroup :label="$ts._pages.script.pageVariables">
            <option v-for="v in hpml.getPageVarsByType()" :value="v">
              {{ v }}
            </option>
          </optgroup>
