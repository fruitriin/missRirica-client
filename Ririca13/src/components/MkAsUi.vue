<template>
  <div>
    ...
    <MkTextarea
      ...
      :model-value="c.default"
      @update:model-value="c.onInput"
    >
      <template v-if="c.label" #label>{{ c.label }}</template>
      <template v-if="c.caption" #caption>{{ c.caption }}</template>
    </MkTextarea>
    ...
    <div
      ...
      :class="[
        $style.container,
        {
          [$style.fontSerif]: c.font === 'serif',
          [$style.fontMonospace]: c.font === 'monospace',
          [$style.containerCenter]: c.align === 'center',
        },
      ]"
      :style="{
        backgroundColor: c.bgColor ?? null,
        color: c.fgColor ?? null,
        borderWidth: c.borderWidth ? `${c.borderWidth}px` : 0,
        borderColor: c.borderColor ?? 'var(--divider)',
        padding: c.padding ? `${c.padding}px` : 0,
        borderRadius: c.rounded ? '8px' : 0,
      }"
    >
      <template v-for="child in c.children" :key="child">
        <MkAsUi
