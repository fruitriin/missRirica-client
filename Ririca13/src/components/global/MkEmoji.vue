...
const customEmojiName = props.emoji
  .substr(1, props.emoji.length - 2)
  .replace("@.", "");
const char = computed(() => (isCustom.value ? undefined : props.emoji));
const useOsNativeEmojis = computed(
  () => defaultStore.state.emojiStyle === "native" && !props.isReaction
);
const url = computed(() => {
  if (char.value) {
...
const alt = computed(() =>
  isCustom.value ? `:${customEmojiName}:` : char.value
);
