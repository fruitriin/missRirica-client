<template>
  ...
  >
    <template #header
      ...
      <template v-if="title"
        >{{ title }}
        <span v-if="typeText" class="turmquns">({{ typeText }})</span></template
    ...
    <section v-else-if="modelValue.type === 'textList'" class="tbwccoaw">
      <textarea
        ...
        :placeholder="$ts._pages.script.blocks._textList.info"
      ></textarea>
    </section>
    ...
    <section v-else-if="modelValue.type === 'ref'" class="hpdwcrvs">
      <select v-model="modelValue.value">
        <option
          v-for="v in hpml
            .getVarsByType(getExpectedType ? getExpectedType() : null)
    ...
    >
      ...
      <XV
        ...
        :title="$t(`_pages.script.blocks._fn.arg1`)"
    ...
    >
      <XV
        ...
        :title="
          hpml.getVarByName(modelValue.type.split(':')[1]).value.slots[i].name
        "
        :get-expected-type="() => null"
        :hpml="hpml"
        :name="name"
    ...
    <section v-else class="" style="padding: 16px">
      <XV
        ...
        :key="i"
        v-model="modelValue.args[i]"
        :title="$t(`_pages.script.blocks._${modelValue.type}.arg${i + 1}`)"
